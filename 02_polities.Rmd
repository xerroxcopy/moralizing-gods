...
`case_when()` extremely slow. took 3min
```{r}

  df  %>% 
  mutate(
    value = case_when(
      # if value note is "range", pick a random value from a normal distribution "Randomly sample from disputed and uncertain, eliminate extra rows"
...
df_join <- left_join(df_min, polities_df %>% select(-NGA), by = "PolID") # 18,823 x 15. drop NGA because polities_df has only one NGA for a PolID, whilst some PolIDs such as AfGrBct extend to other NGAs, hence produces many NA rows.
df_century <- df_join %>% 
  group_by(NGA) %>% 
  summarise(tmin = (min(Start) / 100) %>% ceiling(),
            tmax = (max(End) / 100) %>% floor()
  ) 
