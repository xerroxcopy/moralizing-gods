...
SCdat_df$`Value Note` %>% unique()

df_disputed_uncertain<-
  SCdat_df %>%
  filter(
         `Value Note` %in% c("disputed", "uncertain")) %>% # includes both disputed and uncertain
  group_by(Polity, Variable)  %>% #used to include , `Date From`, `Date To` but seemed to cause unintentional filterings of polities 
...
df_min %>% filter(NGA == "Susiana", Variable == "Professions Professional priesthood")
...
dim(df_spread_prop) # 820 x 71
colnames(output_df_min)
colnames(df_spread_prop)
...
```{r}
left_join(
  df_spread_prop_reordered %>% select(t, NGA, PolID, PropCoded), 
...
df_spread_prop_reordered %>% filter(NGA == "Susiana", t > 600) %>% View()
