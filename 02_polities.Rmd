...
df_join <-  
  ...
  # filter(pmap(., function (t, Start, End, ...) between(t, Start, End))) %>% 
  filter((!is.na(Date) & is.na(DateTo) & (round(Date, -2) == t)) | (!is.na(Date) & !is.na(DateTo) & t >= Date & t <= DateTo) | is.na(Date))# filter out if t is not close to Date, for single date datapoints, OR(|, do not use ||)　filter out if t is out of Date-DateTo range, for ranged date datapoints. This line is very long because consecutive `filter()`s are `and`, not `or` 
dim(df_join) # ~36,980 x 12
df_join %>% filter(PolID == "Hawaii2", t == 1200)# no prof priest it seems
iris %>% filter(between(Sepal.Width, Sepal.Width, Petal.Length ))
```
...
df_join_shortname %>% filter(PolID == "Hawaii2", t == 1200) # no profPriest
```
...
df_spread <- 
  ...
  arrange(NGA, t) # 820 * 77. output's got 817 * 98...
```
